...
static int verify_utf8(const char *src, int sz, int null_term) {
	...
	while (size) {
		...
		if (((*source) & ((unsigned char) 0x80)) == 0) {
			...
			if (size < 2)
				return -1;
			if (((source[1] & ((unsigned char) 0xC0)) != 0x80) || ((*source) < 0xC2) /* overlong */) {
				return -1;
			}
			source += 2;
			size -= 2;
			...
			if (size < 3)
				return -1;
			if (((source[1] & ((unsigned char) 0xC0)) != 0x80) || ((source[2] & ((unsigned char) 0xC0)) != 0x80)
					|| (((*source) == 0xE0) && (source[1] < 0xA0)) /* overlong */) {
				return -1;
			}
			if ((((*source) & ((unsigned char) 0xF)) == 0xD) && ((source[1] & 0x20) != 0)) {
				return -1;
			}
			source += 3;
			size -= 3;
			...
			if ((((*source) & ((unsigned char) 0x7)) > 0x4U)
					|| ((((*source) & ((unsigned char) 0x7)) == 0x4U) && ((source[1] & ((unsigned char) 0x3F)) > 0xFU))) {
				return -1;
			}
			source += 4;
			size -= 4;
...
void run(EpmdVars *g) {
	...
	if (g->addresses != NULL && /* String contains non-separator characters if: */
		...
		for (token = strtok(tmp, ", "), num_sockets = 0; token != NULL; token = strtok(NULL, ", "), num_sockets++) {
	...
	for (i = 0; i < num_sockets; i++) {
		...
		opt = 1;
		if (setsockopt(listensock[i], SOL_SOCKET, SO_REUSEADDR, (char*) &opt, sizeof(opt)) < 0) {
		...
		if (fcntl(listensock[i], F_SETFL, opt | O_NONBLOCK) == -1)
			...
			dbg_perror(g, "failed to set non-blocking mode of listening socket %d", listensock[i]);

		if (bind(listensock[i], (struct sockaddr*) &iserv_addr[i], sizeof(iserv_addr[i])) < 0) {
			if (errno == EADDRINUSE) {
				dbg_tty_printf(g, 1, "there is already a epmd running at port %d", g->port);
...
sd_notifyf(0, "READY=1\n"
	...
	select_again: while (1) {
		...
		if ((ret = select(g->select_fd_top, &read_mask, (fd_set *) 0, (fd_set *) 0, &timeout)) < 0) {
			...
			for (i = 0; i < g->max_conn; i++) {
				if (g->conn[i].open == EPMD_TRUE) {
					if (FD_ISSET(g->conn[i].fd, &read_mask))
						do_read(g, &g->conn[i]);
					else if ((g->conn[i].keep == EPMD_FALSE) && ((g->conn[i].mod_time + g->packet_timeout) < now)) {
...
static void do_read(EpmdVars *g, Connection *s) {
	...
	if (s->keep == EPMD_TRUE) {
		...
		if (val == 0) {
			...
			dbg_tty_printf(g, 1, "error on ALIVE socket %d (%d; errno=0x%x)", s->fd, val, errno);
			node_unreg_sock(g, s->fd);
			epmd_conn_close(g, s);
		} else {
			dbg_tty_printf(g, 1, "got more than expected on ALIVE socket %d (%d)", s->fd, val);
	...
	if (val == 0) {
		...
		dbg_printf(g, 0, "got partial packet only on file descriptor %d (%d)", s->fd, s->got);
		epmd_conn_close(g, s);
		return;
	}
	...
	if ((s->want == 0) && (s->got >= 2)) {
		...
		if (s->got > s->want) {
			dbg_printf(g, 0, "got %d bytes in packet, expected %d", s->got - 2, s->want - 2);
			epmd_conn_close(g, s);
			return;
		}
	}
...
static int do_accept(EpmdVars *g, int listensock) {
	...
	msgsock = accept(listensock, (struct sockaddr*) &icli_addr, (unsigned int*) &icli_addr_len);
...
static void do_request(g, fd, s, buf, bsize)
	EpmdVars *g;int fd;Connection *s;char *buf;int bsize; {
	...
	case EPMD_ALIVE2_REQ:
...

		{
			...
			if ((node = node_reg2(g, namelen, name, fd, eport, nodetype, protocol, highvsn, lowvsn, extralen, extra))
					== NULL) {
				wbuf[1] = 1; /* error */
				put_int16(99, wbuf + 2);
			} else {
...
//			dbg_tty_printf(g, 1, "+++++++++++++++++++++ %lu", node->nodeid);
			put_uint64(node->nodeid, wbuf + 4);
//			dbg_print_buf(g, wbuf, 12);

			if (g->delay_write) /* Test of busy server */
				sleep(g->delay_write);

			if (reply(g, fd, wbuf, 12) != 12) {
				dbg_tty_printf(g, 1, "** failed to send ALIVE2_RESP for \"%s\"", name);
				return;
			}
	...
	case EPMD_NAMES_REQ:
		dbg_printf(g, 1, "** got NAMES_REQ");
		{
			...
			for (node = g->nodes.reg; node; node = node->next) {
				...
				r = erts_snprintf(&wbuf[len], sizeof(wbuf) - len, " at port %d\n", node->port);
				if (r < 0)
					goto failed_names_resp;
				len += r;
				if (reply(g, fd, wbuf, len) != len) {
					failed_names_resp: dbg_tty_printf(g, 1, "failed to send NAMES_RESP");
					return;
				}
			}
		}
	...
	case EPMD_DUMP_REQ:
		...
		}
		{
			...
			for (node = g->nodes.reg; node; node = node->next) {
				...
				len += copy_str(&wbuf[len], node->symname);
				r = erts_snprintf(&wbuf[len], sizeof(wbuf) - len, "> at port %d, fd = %d\n", node->port, node->fd);
				if (r < 0)
					goto failed_dump_resp;
				len += r + 1;
				if (reply(g, fd, wbuf, len) != len) {
					failed_dump_resp: dbg_tty_printf(g, 1, "failed to send DUMP_RESP");
					return;
				}
			}
			...
			for (node = g->nodes.unreg; node; node = node->next) {
				...
				len += copy_str(&wbuf[len], node->symname);
				r = erts_snprintf(&wbuf[len], sizeof(wbuf) - len, ">, port = %d, fd = %d \n", node->port, node->fd);
				if (r < 0)
					goto failed_dump_resp2;
				len += r + 1;
				if (reply(g, fd, wbuf, len) != len) {
					failed_dump_resp2: dbg_tty_printf(g, 1, "failed to send DUMP_RESP");
					return;
				}
			}
		}
...
static int conn_open(EpmdVars *g, int fd) {
	...
	for (i = 0; i < g->max_conn; i++) {
		if (g->conn[i].open == EPMD_FALSE) {
			...
			if (getpeername(s->fd, (struct sockaddr*) &si, &st) || st < sizeof(si)) {
			...
			dbg_tty_printf(g, 2, (s->local_peer) ? "Local peer connected" : "Non-local peer connected");
...
static int node_unreg(EpmdVars *g, char *name) {
	...
	for (; node; prev = &node->next, node = node->next)
		if (is_same_str(node->symname, name)) {
			dbg_tty_printf(g, 1, "unregistering '%s:%d', port %d, sid %d, nodeid %lu", node->symname, node->creation, node->port, node->sid, node->nodeid);

			ffid_releaseid(g->ffid, node->sid);
...
static int node_unreg_sock(EpmdVars *g, int fd) {
	...
	dbg_tty_printf(g, 1, "trying to unregister node with unknown file descriptor %d", fd);
	return -1;
}
...
static Node *node_reg2(EpmdVars *g, int namelen, char* name, int fd, int port, unsigned char nodetype,
		unsigned char protocol, int highvsn, int lowvsn, int extralen, char* extra) {
	...
	if (namelen > MAXSYMLEN) {
		too_long_name: dbg_printf(g, 0, "node name is too long (%d) %s", namelen, name);
		return NULL;
	}
	...
	if (node == NULL) {
		...
		if ((g->nodes.unreg_count > MAX_UNREG_COUNT) || (g->debug && (g->nodes.unreg_count > DEBUG_MAX_UNREG_COUNT))) {
	...
	// nodeid
	node->sid = ffid_getid(g->ffid);
	node->nodeid = ((uint64_t)node->sid << 42) | g->vpmdid;

	select_fd_set(g, fd);

	if (highvsn == 0) {
		dbg_tty_printf(g, 1, "registering '%s:%d', port %d, sid %d, nodeid %lu", node->symname, node->creation, node->port, node->sid, node->nodeid);
	} else {
		dbg_tty_printf(g, 1, "registering '%s:%d', port %d, sid %d, nodeid %lu", node->symname, node->creation, node->port, node->sid, node->nodeid);
		dbg_tty_printf(g, 1, "type %d proto %d highvsn %d lowvsn %d", nodetype, protocol, highvsn, lowvsn);
	}
