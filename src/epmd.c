...
//	gethostname(g->hostname, 127); //sys/param.h, MAXHOSTNAMELEN=64
	...
	while (argc > 0) {
		if ((strcmp(argv[0], "-debug") == 0) || (strcmp(argv[0], "-d") == 0)) {
			...
			if ((argc == 1) || ((g->packet_timeout = atoi(argv[1])) == 0))
				usage(g);
			argv += 2;
			argc -= 2;
		} else if (strcmp(argv[0], "-delay_accept") == 0) {
			if ((argc == 1) || ((g->delay_accept = atoi(argv[1])) == 0))
				usage(g);
			argv += 2;
			argc -= 2;
		} else if (strcmp(argv[0], "-delay_write") == 0) {
			if ((argc == 1) || ((g->delay_write = atoi(argv[1])) == 0))
				usage(g);
			argv += 2;
			argc -= 2;
			...
			if ((argc == 1) || ((g->port = atoi(argv[1])) == 0))
				usage(g);
			argv += 2;
			argc -= 2;
...
static void dbg_gen_printf(int onsyslog, int perr, int from_level, EpmdVars *g, const char *format, va_list args) {
	time_t now;
	char *timestr;
	...
	if (g->is_daemon) {
		...
		erts_snprintf(buf, DEBUG_BUFFER_SIZE, "epmd: %.*s: ", (int) strlen(timestr) - 1, timestr);
		len = strlen(buf);
